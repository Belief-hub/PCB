...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            boxes, (pre_time, det_time, post_time) = det_model(img,
                                                               conf_threshold=conf_threshold,
                                                               iou_threshold=iou_threshold)
            ...
            for (*box, conf, cls_) in boxes:
                # 绘制边界框
                cv2.rectangle(result_img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])),
                              color_palette[int(cls_)], 2, cv2.LINE_AA)

                # 添加中文标签
                label = f'{PCB_CLASSES_CN[int(cls_)]}: {conf:.3f}'
                result_img = draw_chinese_text(result_img, label,
